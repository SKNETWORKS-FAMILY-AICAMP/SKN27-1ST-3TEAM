...
보조금 정책 분석 kpi : 지역의 전기차, 수소차의 보조금 종류별 합을 전국 평균으로 나눈 값을 kpi를 통해 확인
지역별 보조금 및 전기차 등록 현환 chart:  지역별 전기차, 수소차의 보조금합을 비교한 데이터를 bar_chart로 확인 가능, 전국 전기차 등록 수(2024년 기준)을 차트에서 시각적으로 확인 가능 
자주 묻는 질문 (FAQ) text_input :  질문을 검색하여 데이터 베이스에 있는 내용 도출
자주 묻는 질문 (FAQ) 카테고리 : 클릭을 통해 카테고리 전환 & 접힌탭 펼쳐 내용 확인 가능
